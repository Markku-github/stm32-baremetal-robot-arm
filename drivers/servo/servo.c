...
servo_status_t servo_angle_rad_to_pulse_us(const servo_t *servo, float angle_rad, uint16_t *pulse_width_us)
{
    ...
    pulse_range_us = (float)(servo->maximum_pulse_width_us - servo->minimum_pulse_width_us);
    normalized_position = (adjusted_angle_rad - servo->minimum_angle_rad) / angle_range_rad;
    pulse_width_f = (float)servo->minimum_pulse_width_us + (normalized_position * pulse_range_us);

    if (pulse_width_f < (float)servo->minimum_pulse_width_us)
    {
        pulse_width_f = (float)servo->minimum_pulse_width_us;
    }
    else if (pulse_width_f > (float)servo->maximum_pulse_width_us)
    {
        pulse_width_f = (float)servo->maximum_pulse_width_us;
    }
